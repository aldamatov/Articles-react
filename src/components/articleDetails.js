...
function ArticleDetails({ users }) {
  const url = useParams();
  const filtered = data.filter((article) => article.name === url.name)[0];
  const restOfTheArticles = data.filter((article) => article.name !== url.name);
  return (
    <div>
      <div>{filtered.contents[0].props.children}</div>
      <br />
